...
void ResourceManager::parseResourceGroup(TiXmlElement *elem)
{
	...
	for (TiXmlElement *child = elem->FirstChildElement() ; child!=NULL ; child = child->NextSiblingElement())
	{
		...
		{
			if (const char* attr = child->Attribute("idprefix")) idPrefix = attr;
			else idPrefix = "";
			if (const char* attr = child->Attribute("path")) basePath = attr;
			else basePath = "";
		...
		else if (Boy::Environment::instance()->stricmp(val, "Directory") == 0)
		{
			std::string dirPath = basePath;
			dirPath.append(child->Attribute("path"));
			// add trailing slash if necessary:
			...
			// remove "./" from begining if it's there:
			if (dirPath.substr(0,2)=="./" ||
				dirPath.substr(0,2)==".\\")
			{
				dirPath = dirPath.substr(2);
			}
			// parse directory
			parseDirectory(child, dirPath, group);
		}
		else
		{
...
void ResourceManager::parseDirectory(TiXmlElement *elem, const std::string &path, ResourceGroup *group)
{
	...
	for (TiXmlElement *child = elem->FirstChildElement() ; child!=NULL ; child = child->NextSiblingElement())
	{
		...
		if (Boy::Environment::instance()->stricmp(val, "SetDefaults") == 0)
		{
			...
			basePath = path;
			if (const char* attr = child->Attribute("path")) basePath.append(attr);
