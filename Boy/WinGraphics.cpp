...
void WinGraphics::mapZ(float z0, float z1, float mapz0, float mapz1)
{
	// maps [z0, z1] to [mapz0, mapz1]
	if (z0 == z1) return;
	float zScale = (mapz1 - mapz0) / (z1 - z0);
	D3DXMATRIX xformTrans1, xformScale, xformTrans2;
	D3DXMatrixTranslation(&xformTrans1, 0, 0, -z0);
	D3DXMatrixScaling(&xformScale, 1, 1, zScale);
	D3DXMatrixTranslation(&xformTrans2, 0, 0, mapz0);
	mTransformStack.top() = xformTrans2 * xformScale * xformTrans1 * mTransformStack.top();
	mTransformUpToDate = false;
}
